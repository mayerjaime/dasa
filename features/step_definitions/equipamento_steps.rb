When('executo a chamada no servico de equipamentos informando um protocolo {string}') do |tp_protocol|
  ...
  @equipamento.cadastra_equipamento(tp_protocol)
end
...
Then('recebo a mensagem de sucesso.') do
  msg_equip_sucesso = $param_messages['equipamento_cadastrado_sucesso'].gsub('<nome_equip>', $faker_equip)
  expect($cad_equip.gsub(/[']/, '')).to eql(msg_equip_sucesso)
end

Then('a mensagem de protocolo nao suportado.') do
  msg_equip_nao_suportado = $param_messages['equipamento_nao_suportado'].gsub('<nome_protocolo>', 'JMTPDAPI')
